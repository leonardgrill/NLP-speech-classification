...
try:
    ...
    # For scikit - learn : Average MFCCs over time mfcc
    mfccs_mean = np.mean(mfccs , axis=1)
    X.append(mfccs_mean)
    y.append(label)
except Exception as e :
print ( f ”Error processing {file_path }: {e}”)
# Convert to numpy arrays after collecting a l l f i l e s
X = np. array(X)
y = np. array(y)
i f X. size == 0:
...
X_train = X
y_train = y_encoded
n_features = X. shape [ 1 ] i f X.ndim == 2 else 1
...
# I f there i s 0 or 1 sample l e f t in X_temp, assign appropriately
i f len (X_temp) == 0:
...
print ( f ”Training data Shape ( scikit - learn ) : {X_train_sk. shape} , Labels :
{y_train_sk . shape}”)
